...
def _worker(args):
    """Load one file, compute per-file audio + spec running sums.
    ...
    """
    filepath, sample_rate, max_seconds, n_fft, hop_length, hf_cut = args
    try:
...
def main():
    ...
    tasks = [
        (fp, args.sample_rate, args.max_seconds, args.n_fft, args.hop_length,
         args.hf_cut)
        for fp in filepaths
    ]
    ...
    log.info("Spawning %d workers over %d files ...",
             args.num_workers, len(tasks))
